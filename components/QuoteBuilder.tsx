...
const ItemDetailEditor: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-2xl z-[250] flex items-center justify-center p-0 animate-in fade-in duration-500" onClick={onCancel}>
      <div className="bg-white w-full h-full md:h-[98vh] md:w-[98%] md:rounded-[3.5rem] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500 border-x-4 border-slate-900" onClick={e => e.stopPropagation()}>

        {/* Enhanced Header with Stats */}
        <div className="p-6 border-b border-slate-100 bg-white">
          {/* Unified Top Header */}
          <div className="p-8 border-b-4 border-slate-900 bg-white flex justify-between items-center shrink-0 z-20 shadow-md">
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 bg-slate-900 text-amber-500 rounded-3xl flex items-center justify-center font-black text-3xl shadow-xl border-2 border-amber-500/20">
                {localItem.type.charAt(0)}
              </div>
              <div>
                <h3 className="font-black text-slate-900 text-3xl tracking-tighter uppercase leading-none mb-1">Editor Profissional</h3>
                <p className="text-sm text-slate-400 font-black uppercase tracking-widest">{localItem.observation || localItem.type} • ID {localItem.id.slice(0, 4)}</p>
              </div>
            </div>

            <div className="flex gap-6">
              <div className="bg-slate-50 px-8 py-3 rounded-full border-2 border-slate-100 flex flex-col items-center">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Peça base</span>
                <span className="text-xl font-black text-slate-800 tracking-tight">{(localItem.length * 100).toFixed(0)}cm</span>
              </div>
              <div className="bg-indigo-50 px-8 py-3 rounded-full border-2 border-indigo-100 flex flex-col items-center">
                <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">Peso Estimado</span>
            ...
            <div className="flex gap-4">
              <button onClick={handleSaveAll} className="px-10 py-5 rounded-[2rem] bg-emerald-500 text-white text-sm font-black uppercase tracking-widest hover:bg-emerald-600 shadow-xl transition-all active:scale-95 border-b-4 border-emerald-700">Salvar Tudo</button>
              <button onClick={onCancel} className="p-5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-[2rem] transition-all border-2 border-transparent hover:border-red-100 shadow-sm bg-slate-50">
            ...
            <div className="flex-grow flex flex-col overflow-hidden bg-slate-50/50">
              ...
              {/* AREA 1: FULL WIDTH LONGITUDINAL DETAILING */}
              {!isSapata && (
                <div className="h-[45%] shrink-0 p-10 bg-white border-b-4 border-slate-900/5 overflow-hidden flex flex-col relative">
                  <div className="flex justify-between items-center mb-6 px-10">
                    <div className="flex items-center gap-4">
                      <span className="bg-indigo-600 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">Espaço Interativo</span>
                      <h4 className="text-sm font-black text-slate-900 tracking-tight uppercase">Detalhamento para Fabrico (Blueprint HD)</h4>
                    </div>
                    <div className="flex gap-4 items-center">
                      <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-red-400" /><span className="text-[9px] font-black text-slate-500 uppercase">Negativo</span></div>
                      <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-slate-800" /><span className="text-[9px] font-black text-slate-500 uppercase">Positivo</span></div>
                      <div className="h-4 w-[1px] bg-slate-200 mx-2" />
                      <p className="text-[10px] font-bold text-slate-400 italic">Dica: Selecione a barra no desenho para editar suas dimensões abaixo</p>
                    </div>
                  </div>

                  <div className="flex-grow flex items-center justify-center relative bg-indigo-50/5 rounded-[4rem] border-2 border-dashed border-slate-200 p-8 shadow-inner overflow-x-auto custom-scrollbar">
                    <div className="min-w-[1600px] flex justify-center">
                      <BeamElevationView
                        item={localItem}
              ...
              {/* BOTTOM GRID: Cross-Section & Bar Form */}
